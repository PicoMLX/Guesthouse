...
Bind each check to the environment, connection/session generation, relevant capability revision, tool identity and current observation request. Invalidation and a superseding check revoke the earlier request's ability to publish. A late success must not restore readiness after reconnect or permission loss, and a late failure must not overwrite newer verified evidence. Test these orderings explicitly, including tool replacement and wrong-environment results; reuse the existing identity/ordering contracts rather than relying on timestamps alone.
...
Test explicit wrong-host, wrong-guest/session and stale-target requests after reconnect against benign fixtures. The supported Guesthouse path must refuse or report unavailable before capture/action. An unchanged host marker is supporting evidence, not containment proof. Record any wider authority of the underlying tools separately; this path's refusal is not global Codex isolation. An unenforceable target boundary requires a recorded owner decision.
...
The content policy begins before capture and transmission to the model, including transient observations never saved as artifacts. Define approved apps/windows, user disclosure that visible content may be model-processed, and behavior on revoked permission. Exclude authentication/credential/unrelated windows or require deliberate user involvement before proceeding; if scope cannot be enforced, refuse or report unavailable. Use benign fixture content during research. Later masking, retention limits or export consent cannot retroactively protect an already transmitted observation. [Computer Use data guidance](https://learn.chatgpt.com/docs/computer-use).
...
Prioritize preliminary human feasibility checks for a strictly valid provider artifact, actual guest Codex connectivity and native guest MLX/GPU execution as soon as each has a reviewed, safe, bounded diagnostic path. They need not wait for the complete persistence/dashboard implementation or full fixture. Keep strict verification, owned-process/storage safety and required SSH/credential/console protections. Record exact versions, operator, scope and limitations under `docs/research/provider-feasibility.md`; these are candidate observations, not provider acceptance or formal #38/#41/#42 results. Build only the safety prerequisites required for each check and retain all later gate requirements.

Time-box #188 to two engineer-days plus one human validation session of at most two hours once its safe prerequisites are available. Ronald Mannak owns the scope decision. Record the start/effort and stop at exhaustion with a build/test-only recommendation or a bounded extension proposal; do not automatically pursue all integrations or renew the budget. #42 needs the explicit scope decision, not successful implementation of every candidate path. No response or elapsed time is approval, and a build/test-only decision waives none of the existing gates.
