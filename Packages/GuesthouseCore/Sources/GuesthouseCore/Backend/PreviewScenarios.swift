...
/// Retained #60 fixtures: dates/tool values are synthetic, never hardware or provider evidence.
...
public enum PreviewScenarios: Sendable {
    ...
    /// One simulated running environment with sample tool versions; no provider is verified.
    public static func oneRunningEnvironment() async -> PreviewScenario {
        ...
        await backend.setStatus(EnvironmentStatus(
            ...
            observed: ObservedTuple(hostMacOSVersion: SemanticVersion("26.5.2"), guestMacOSBuild: "25F84", xcodeBuild: "17F113"),
            reconciledAt: date
        ))
    ...
    private static func snapshot(for environments: [DevelopmentEnvironment], provisioning: [EnvironmentID: ProvisioningState]) throws -> EnvironmentsSnapshot {
        ...
        let snapshot = EnvironmentsSnapshot(environments: environments, slots: slots, provisioning: provisioning)
