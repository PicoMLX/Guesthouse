...
public enum GuesthouseError: Error, Codable, Hashable, Sendable {
    ...
    // Share the complete bounded sanitizer with its immutable value type.
    static let sanitizeLookahead = SanitizedText.sanitizeLookahead

    public static func sanitize(_ value: String, limit: Int = 80) -> String {
        SanitizedText.sanitize(value, limit: limit)
    }

    static func inspectionWindow<S: Sequence>(
        _ scalars: S, maximum: Int
    ) -> (value: String, truncated: Bool) where S.Element == Unicode.Scalar {
        SanitizedText.inspectionWindow(scalars, maximum: maximum)
    }
