...
extension Redactor {
    private static let credentialFieldPrefixes: Set<String> = {
        let names = ["authorization", "proxy authorization", "request authorization", "cookie", "cookies", "set cookie", "set cookies", "request cookie", "request cookies",
                     "password", "passphrase", "passwd", "secret", "token", "credential", "credentials", "api key", "private key", "secret key", "secret access key", "access key secret", "device code", "user code", "device codes", "user codes"]
        let fields = names.flatMap { name in ["", "-", "_", " "].map { name.replacingOccurrences(of: " ", with: $0) } }
    ...
    private static let providerStems = ["ghp_", "gho_", "ghu_", "ghs_", "ghr_", "github_pat_"]
    ...
    static func partialCredentialLabel(in text: String) -> String? {
        ...
        if let header = text.firstMatch(of: #/(?:^|[^A-Za-z0-9_-])([A-Za-z][A-Za-z0-9_-]{0,47})[ \t]*$/#) {
            let prefix = String(header.1).lowercased()
            if credentialFieldPrefixes.contains(prefix)
                || authorizationSchemes.contains(where: { $0.hasPrefix(prefix) && $0 != prefix }) { return prefix }
        }
        let tail = String(text.suffix(11))
        let providerPrefixes = providerStems.flatMap { stem in (1..<stem.count).map { String(stem.prefix($0)) } }
        if let prefix = providerPrefixes.filter({ tail.hasSuffix($0) }).max(by: { $0.count < $1.count }) { return prefix }
        return nil
    }
