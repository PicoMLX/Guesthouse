...
@Suite struct ProvisioningOperationIdentityTests {
    ...
    @Test func resumeEvidenceSurvivesARejectedStart() throws {
        let evidence = try #require(ResumeEvidence(kind: .partialDownload, stagingPath: "downloads/restore.ipsw.partial"))
    ...
    /// Compatible schema-2 tokenless reservations are inspection-only. These are not a
    /// migration of schema-1 raw-summary records; the model tests require those to be refused.
    @Test(arguments: [
        #"{"schemaVersion":2,"stage":"preflight","issuedEffects":9,"status":{"startRequested":{}}}"#,
        #"{"schemaVersion":2,"stage":"preflight","issuedEffects":9,"status":{"startRequested":{"resuming":{"kind":"partialDownload","stagingPath":"downloads/restore.ipsw.partial"}}}}"#,
    ])
    func tokenlessStartReservationsCanOnlyRecoverThroughInspection(record: String) throws {
        ...
        if record.contains("resuming") {
            let expected = try #require(ResumeEvidence(kind: .partialDownload, stagingPath: "downloads/restore.ipsw.partial"))
        ...
        for reply in [
            ...
            #expect(throws: ProvisioningTransitionError.illegalTransition(status: .startRequested, event: reply.kind)) {
    ...
    @Test func startRequestTokensAdvanceThePersistedCounter() throws {
        let record = Data(#"{"schemaVersion":2,"stage":"preflight","issuedEffects":0,"status":{"startRequested":{"request":9}}}"#.utf8)
