...
Every gate above has status `passed` or `passed with follow-ups`, each with a record of the exact versions used, and the complete-path run reports total setup time, active user time, peak disk and memory, and the number of guest-console interventions. A `failed` gate blocks phase 0: `MVP-PLAN.md` §10 requires resolving a failed lifecycle, pairing, cold-boot, or GPU gate before expanding the UI. The only way past a failed gate is an explicit, approved exception recorded as an ADR in [`docs/decisions/`](../decisions/README.md) that states what was not proven and why work continues anyway. Decisions that came out of the gates are also written as ADRs. Only then are the Phase 2 to 5 epics split into implementation issues.
