...
`exception approved` exists only for the two gates where `MVP-PLAN.md` §10 allows a scope or architecture decision instead of a fix: the native MLX/GPU gate (#38, "obtain a scope or architecture decision") and the local-override part of the multi-repo gate (#39, "narrow the supported project shape explicitly"). It requires an ADR in `docs/decisions/` that records the decision, linked from the gate record. Every other gate must reach `passed` or `passed with follow-ups`: §10 says first-boot UI, runtime ownership, secure cold-boot authentication, and GUI-only pairing failures are resolved before the UI expands, not waived.
